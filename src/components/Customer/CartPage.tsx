...
import { Minus, Plus, Trash2, ShoppingCart, ArrowLeft, AlertCircle, CheckCircle } from 'lucide-react';
...
export const CartPage: React.FC = () => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // Try with minimal order first (without order items)
      const minimalOrderData: Omit<OrderDTO, 'id'> = {
        customerId: Number(user.id),
        customerName: user.name,
        total: cartTotal,
        status: 'PENDING',
        ...
        tableNumber: orderType === 'DINE_IN' ? tableNumber : undefined,
        notes: notes.trim() || undefined,
        orderItems: [] // Empty array to test if order creation works
      };

      console.log('Trying minimal order first...');
      const orderData = minimalOrderData;
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-blue-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-lg mb-6">
          ...
          <p>Cart Items: {JSON.stringify(cart.map(item => ({ name: item.name, price: item.price, quantity: item.quantity })))}</p>
        </div>
